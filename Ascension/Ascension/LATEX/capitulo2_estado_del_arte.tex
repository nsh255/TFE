...
La Figura \ref{fig:ea-rogue} muestra una captura de pantalla del videojuego original \textit{Rogue} y su interfaz basada en caracteres ASCII.
...
\begin{enumerate}
\item \textbf{Generación procedural de niveles.} La estructura espacial del juego se genera algorítmicamente en cada partida, de modo que no existen dos ejecuciones idénticas. Esta característica implica la necesidad de diseñar algoritmos capaces de producir entornos coherentes, equilibrados y accesibles sin intervención manual.
...
\item \textbf{Complejidad de sistemas interconectados.} Los \textit{roguelikes} suelen presentar múltiples sistemas que interactúan entre sí de forma emergente, generando situaciones impredecibles que enriquecen la experiencia de juego.
\end{enumerate}
...
La Figura \ref{fig:ea-steamdb-roguelike} muestra la evolución del número de títulos \textit{roguelike} publicados en Steam por año.
...
\label{fig:ea-steamdb-roguelike}
\end{center}
\end{figure}
...
\caption{Comparativa entre \textit{roguelike} clásico y \textit{roguelite}.}
\end{table}
...
\caption{Comparativa de técnicas de IA para enemigos en videojuegos 2D.}
\end{table}
...
La Figura \ref{fig:ea-isaac} muestra un ejemplo de perspectiva cenital y estructura de salas en \textit{The Binding of Isaac}.
...
La Figura \ref{fig:ea-gungeon} muestra la mecánica de esquiva (\textit{dodge roll}) durante el combate y la variedad de proyectiles en pantalla.
...
La Figura \ref{fig:ea-hades} muestra el combate en tiempo real, la presencia simultánea de enemigos y la interfaz de estado del personaje en \textit{Hades}.
...
\textbf{Características relevantes para \textit{Ascension}:}
\begin{itemize}
\item \textbf{Combate ágil y fluido.} \textit{Dead Cells} se caracteriza por un combate rápido y preciso. En \textit{Ascension} se han implementado controles fluidos y tiempos de espera (\textit{cooldown}) reducidos con el objetivo de aproximarse a este enfoque.
...
La Figura \ref{fig:ea-deadcells} muestra el combate lateral característico de \textit{Dead Cells}.
...
\caption{Comparativa entre referencias comerciales y el proyecto \textit{Ascension}.}
\end{table}
...
\begin{enumerate}
\item \textbf{Sistema de tiles con efectos.} Ninguno de los títulos analizados incorpora un sistema de tiles especiales que modifiquen las condiciones de combate en tiempo real. Se incorporó este sistema en \textit{Ascension} para resolver un problema de diseño concreto: en un \textit{roguelike} con tres tipos de enemigos y un repertorio limitado de armas, la variabilidad táctica de cada sala depende exclusivamente de la composición de la oleada de enemigos, lo que puede generar repetitividad. Los tiles con efectos (hielo (incremento de velocidad), lodo (reducción de velocidad), curación y potenciación de daño) introducen una segunda dimensión de variabilidad: el entorno físico de la sala condiciona las decisiones tácticas del jugador. La alternativa descartada era añadir más tipos de enemigo, pero esto habría incrementado significativamente el volumen de arte y animación necesario.
...
La Figura \ref{fig:ea-ascension-combate} muestra una partida de \textit{Ascension} donde se aprecia la perspectiva cenital, los tiles especiales y la interfaz de corazones.
...
\label{fig:ea-ascension-combate}
\end{center}
\end{figure}


La Figura \ref{fig:ea-ascension-clase} muestra la pantalla de selección de clase de \textit{Ascension}.
...
\caption{Comparativa de motores de desarrollo de videojuegos.}
\end{table}
...
\caption{Correspondencia entre funcionalidades de Unity/C\# utilizadas en el proyecto y sus equivalentes en motores alternativos.}
\end{table}
...
\caption{Herramientas complementarias utilizadas en el desarrollo.}
\end{table}
...
\caption{Comparativa de herramientas para producción de gráficos 2D.}
\end{table}
...
La Figura \ref{fig:ea-libresprite} muestra la interfaz de LibreSprite durante la edición de un \textit{sprite}.
