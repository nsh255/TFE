...
La Figura \ref{fig:desarrollo-gamemanager-estados} muestra el diagrama de estados UML del \texttt{GameManager}.
...
\label{fig:desarrollo-gamemanager-estados}
\end{center}
\end{figure}
...
\caption{Eventos del \texttt{GameManager}.}
\end{table}
...
La Figura \ref{fig:desarrollo-coordinacion-startnewrun} resume la cadena de coordinación entre gestores al iniciar una nueva partida.
...
\label{fig:desarrollo-coordinacion-startnewrun}
\end{center}
\end{figure}
...
La Figura \ref{fig:desarrollo-flujo-completar-sala} muestra el flujo completo de una sala (desde la aparición de enemigos hasta la transición a la siguiente sala) y la cadena de invocaciones entre gestores.
...
\label{fig:desarrollo-flujo-completar-sala}
\end{center}
\end{figure}


La Figura \ref{fig:desarrollo-menu-principal} muestra el menú principal del videojuego.
...
\label{fig:desarrollo-menu-principal}
\end{center}
\end{figure}
...
El \texttt{RoomFlowController} gestiona el bucle central de generación y progresión de salas. La Figura \ref{fig:desarrollo-roomflow-estados} describe su ciclo de funcionamiento.
...
\label{fig:desarrollo-roomflow-estados}
\end{center}
\end{figure}
...
\begin{itemize}
\item \textbf{Línea 1:} Si el jugador está ejecutando una acción evasiva, se ignora toda la entrada de movimiento; el \textit{roll} tiene su propia lógica de velocidad gestionada por la corutina.
\item \textbf{Líneas 2-3:} \texttt{GetAxisRaw} devuelve -1, 0 o 1 sin suavizado de transición, lo que produce un movimiento inmediato y reactivo. Se seleccionó \texttt{GetAxisRaw} frente a \texttt{GetAxis} porque este último aplica un suavizado temporal que introduce latencia perceptible entre la pulsación de tecla y el movimiento, inadecuado para un \textit{roguelike} de acción en tiempo real donde la precisión de movimiento es crítica.
...
La Figura \ref{fig:desarrollo-mecanica-esquiva} resume el funcionamiento de la mecánica de esquiva.
...
\label{fig:desarrollo-mecanica-esquiva}
\end{center}
\end{figure}
...
La Figura \ref{fig:desarrollo-ciclo-vida-jugador} muestra el ciclo de vida del jugador y los estados por los que transita.
...
\label{fig:desarrollo-ciclo-vida-jugador}
\end{center}
\end{figure}
...
\caption{Estados, transiciones y acciones de la FSM del SlimeGreen.}
\end{table}

La Figura \ref{fig:desarrollo-fsm-slimegreen} formaliza las transiciones entre los tres estados del SlimeGreen.

\begin{figure}[H]
\begin{center}
\includegraphics[width=0.525\textwidth,height=0.21\textheight,keepaspectratio]{Figuras/fsm_slimegreen.png}
\caption{
Diagrama de la FSM del SlimeGreen. \textit{Elaboración propia.}
}
\label{fig:desarrollo-fsm-slimegreen}
\end{center}
\end{figure}
...
\caption{Estados y transiciones de la FSM del SlimeBlue.}
\end{table}

La Figura \ref{fig:desarrollo-fsm-slimeblue} muestra el diagrama de estados de la FSM del SlimeBlue.

\begin{figure}[H]
\begin{center}
\includegraphics[width=0.525\textwidth,height=0.21\textheight,keepaspectratio]{Figuras/fsm_slimeblue.png}
\caption{
Diagrama de la FSM del SlimeBlue. \textit{Elaboración propia.}
}
\label{fig:desarrollo-fsm-slimeblue}
\end{center}
\end{figure}
...
La Figura \ref{fig:desarrollo-enemigo-jefe} muestra un encuentro con un enemigo jefe, destacando el incremento visual de tamaño respecto a los enemigos estándar.
...
\label{fig:desarrollo-enemigo-jefe}
\end{center}
\end{figure}
...
\caption{Comparativa de comportamiento, parámetros y rol táctico de los tres tipos de enemigo.}
\end{table}
...
La Figura \ref{fig:desarrollo-flujo-decision-ia} muestra la lógica de decisión completa que ejecuta cada tipo de enemigo en cada fotograma, incluyendo la lógica heredada de la clase base \texttt{Enemy} (común a los tres) y la lógica específica de cada subclase.
...
\label{fig:desarrollo-flujo-decision-ia}
\end{center}
\end{figure}
...
La Figura \ref{fig:desarrollo-clases-enemigos} muestra el diagrama de clases UML de la jerarquía de enemigos.
...
\label{fig:desarrollo-clases-enemigos}
\end{center}
\end{figure}


La Figura \ref{fig:desarrollo-secuencia-combate} muestra el diagrama de secuencia UML del flujo de combate.

\clearpage
\begin{sidewaysfigure}
\centering
\includegraphics[width=\textheight,keepaspectratio]{Figuras/figura4_6_secuencia_combate.png}
\caption{
Diagrama de secuencia UML del flujo de combate. \textit{Elaboración propia.}
}
\label{fig:desarrollo-secuencia-combate}
\end{sidewaysfigure}
\clearpage
...
El sistema de combate involucra a cinco componentes distribuidos en tres subsistemas (armas, enemigos y gestión central). La Figura \ref{fig:desarrollo-flujo-dano} describe la cadena completa de invocaciones que se produce cuando el jugador ataca a un enemigo, desde la entrada del jugador hasta la actualización de la puntuación:
\vspace{-0.5em}
\begin{figure}[H]
\begin{center}
...
\label{fig:desarrollo-flujo-dano}
\end{center}
\end{figure}
\vspace{-0.5em}
...
\vspace{-0.5em}
La Figura \ref{fig:desarrollo-clases-armas} muestra el diagrama de clases UML del sistema de armas.
...
\label{fig:desarrollo-clases-armas}
\end{center}
\end{figure}
...
public void GenerateRoom() {
    bool clearWalls = true; // En el proyecto real, esta decisión se pasa como parámetro.
    ...
    if (!clearWalls) return; // Caso RegenerateFloorKeepWalls(): no se repintan muros.
...
\textbf{Probabilidades de aparición de tiles especiales:} La Tabla \ref{tab:probabilidades-tiles} resume las probabilidades, el formato de distribución y la cantidad por sala.
\vspace{-0.5em}
\begin{table}[H]
\centering
{\small
...
\caption{Probabilidades y formato de distribución de tiles especiales.}
\label{tab:probabilidades-tiles}
\end{table}
\vspace{-0.5em}

La Figura \ref{fig:desarrollo-secuencia-generacion-sala} muestra el diagrama de secuencia UML de la generación procedural de una sala.
...
\label{fig:desarrollo-secuencia-generacion-sala}
\end{center}
\end{figure}
...
La Figura \ref{fig:desarrollo-secuencia-progresion-salas} muestra el diagrama de secuencia UML del ciclo de progresión entre salas.

\clearpage
\begin{sidewaysfigure}
    \begin{center}
    \includegraphics[width=\textheight]{Figuras/figura4_11_secuencia_progresion_salas.png}
    \caption{
    Diagrama de secuencia UML del ciclo de progresión entre salas. \textit{Elaboración propia.}
    }
    \label{fig:desarrollo-secuencia-progresion-salas}
    \end{center}
\end{sidewaysfigure}
\clearpage
...
La Figura \ref{fig:desarrollo-menu-pausa} muestra el menú de pausa de \textit{Ascension}.
...
\label{fig:desarrollo-menu-pausa}
\end{center}
\end{figure}
