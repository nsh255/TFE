...
En este contexto se plantea el siguiente problema técnico: cómo diseñar e implementar una arquitectura de software modular que permita la generación automática de niveles funcionables, la gestión concurrente de múltiples subsistemas (como la inteligencia artificial de los enemigos, el motor de físicas, la interfaz de usuario (UI) o el control de estados del juego) y la parametrización del contenido de manera escalable y mantenible. Este problema resulta representativo de los retos que afronta la ingeniería del software aplicada al desarrollo de sistemas interactivos en tiempo real.

En el marco de este trabajo, la inteligencia artificial de los enemigos se implementa como un sistema propio basado en máquinas de estados finitos (FSM) y eventos en C\#, sin depender de un módulo de IA predefinido del motor Unity.
...
La Figura \ref{fig:gantt-planificacion} muestra la planificación temporal del proyecto, con la distribución de las ocho fases, su dedicación en horas y la superposición parcial entre algunas de ellas.
