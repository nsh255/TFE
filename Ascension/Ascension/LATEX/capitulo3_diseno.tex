...
\caption{Glosario de términos técnicos utilizados en este capítulo.} \\
\hline
...
\textbf{Aclaración terminológica:} en este documento, el término \textit{inteligencia artificial} hace referencia a la lógica de control desarrollada específicamente para los enemigos (scripts en C\# basados en máquinas de estados finitos (FSM) y eventos). Unity proporciona el entorno de ejecución (ciclo \texttt{Update()}, físicas y colisiones), pero no se utiliza un sistema de IA \textit{integrado} del motor.
...
La Figura \ref{fig:diseno-arquitectura-componentes} resume los componentes principales de la arquitectura y su relación.

\clearpage
\begin{sidewaysfigure}
\centering
\includegraphics[width=\textheight,keepaspectratio]{Figuras/figura3_1_arquitectura_componentes.png}
\caption{
Diagrama de componentes de la arquitectura general del sistema. \textit{Elaboración propia.}
}
\label{fig:diseno-arquitectura-componentes}
\end{sidewaysfigure}
\clearpage
...
La Figura \ref{fig:diseno-flujo-escenas} muestra el flujo de escenas del proyecto.

\begin{figure}[H]
\begin{center}
\includegraphics[width=0.525\textwidth,height=0.21\textheight,keepaspectratio]{Figuras/flujo_escenas_simple.png}
\caption{
Diagrama del flujo de escenas. \textit{Elaboración propia.}
}
\label{fig:diseno-flujo-escenas}
\end{center}
\end{figure}
...
\caption{Gestores centrales del sistema.}
\end{table}
...
La Figura \ref{fig:diseno-dependencias-modulos} muestra las dependencias entre los módulos principales del sistema.

\clearpage
\begin{sidewaysfigure}
\centering
\includegraphics[width=\textheight,keepaspectratio]{Figuras/dependencias_modulos.png}
\caption{
Diagrama de dependencias entre los módulos del sistema. \textit{Elaboración propia.}
}
\label{fig:diseno-dependencias-modulos}
\end{sidewaysfigure}
\clearpage
...
La Figura \ref{fig:diseno-importacion-sprites} muestra la configuración de importación de \textit{sprites} en Unity.
...
\label{fig:diseno-importacion-sprites}
\end{center}
\end{figure}
...
La Figura \ref{fig:diseno-componentes-jugador} muestra los componentes del \textit{GameObject} del jugador en el inspector de Unity.
...
\label{fig:diseno-componentes-jugador}
\end{center}
\end{figure}
...
La Figura \ref{fig:diseno-componentes-enemigo} muestra los componentes de un enemigo en el inspector de Unity.
...
\label{fig:diseno-componentes-enemigo}
\end{center}
\end{figure}
...
La Figura \ref{fig:diseno-editor-tilemaps} muestra el editor de \textit{tilemaps} con las capas de muros y suelo.
...
\label{fig:diseno-editor-tilemaps}
\end{center}
\end{figure}
...
La Figura \ref{fig:diseno-animator-controller} muestra el controlador de animación del jugador y sus transiciones principales.
...
\label{fig:diseno-animator-controller}
\end{center}
\end{figure}
...
El \texttt{PlayerController} interactúa con cualquier arma con la interfaz \texttt{Weapon.Attack()}, sin necesidad de conocer el tipo concreto. Añadir un nuevo tipo de arma solo requiere crear una nueva subclase sin modificar el código existente.
...
La Figura \ref{fig:diseno-composicion-jugador} resume la composición del jugador basada en componentes.
...
\label{fig:diseno-composicion-jugador}
\end{center}
\end{figure}
...
La Figura \ref{fig:diseno-estados-juego} muestra la máquina de estados del juego y sus transiciones.

\begin{figure}[H]
\begin{center}
\includegraphics[width=0.525\textwidth,height=0.21\textheight,keepaspectratio]{Figuras/estados_juego.png}
\caption{
Diagrama de los estados del juego. \textit{Elaboración propia.}
}
\label{fig:diseno-estados-juego}
\end{center}
\end{figure}
